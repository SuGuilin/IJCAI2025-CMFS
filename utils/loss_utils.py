...
class FusionLoss(nn.Module):
    ...
    def forward(self, input_xy, output, Mask):
        ...
        R_vis = torchvision.transforms.functional.adjust_gamma(input_vis, self.cfg.Gamma_coeff, 1)
        ...
        R_ir = torchvision.transforms.functional.adjust_contrast(input_ir, self.cfg.Contrast_coeff)
        ...
        fusion_loss_total = self.cfg.alpha * con_loss  + self.cfg.beta * gradient_loss  + self.cfg.gamma * color_loss

        return fusion_loss_total

class MultiTaskLoss(nn.Module):
    def __init__(self, background, cfg):
        super(MultiTaskLoss, self).__init__()

        self.semantic_loss = nn.CrossEntropyLoss(reduction='mean', ignore_index=background)
        self.FusionLoss = FusionLoss(cfg=cfg)
        self.cfg = cfg
    ...
    def forward(self, inputs, outputs, Mask, label):
        ...
        loss = self.cfg.theta * semantic_loss_total + self.cfg.sigma * fusion_loss_total
